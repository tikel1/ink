...
int HttpContainerArduino::read(uint8_t *buf, size_t max_len) {
  ...
  // Ink patch: the body may not be buffered the instant after the response
  // headers arrive (or between TCP segments mid-download), so available() can
  // briefly be 0. Upstream returns 0 here and the OTA loop treats that as EOF,
  // exiting after only the first burst (~4 KB) -> truncated image -> MD5
  // mismatch. Wait for more data while the stream is open and we still expect
  // bytes; only a genuine close/timeout yields 0 (true end of data).
  int available_data = stream_ptr->available();
  if (available_data == 0 && this->bytes_read_ < this->content_length) {
    const uint32_t wait_start = millis();
    while (available_data == 0 && stream_ptr->connected() && (millis() - wait_start) < 10000) {
      App.feed_wdt();
      delay(2);
      available_data = stream_ptr->available();
    }
  }
